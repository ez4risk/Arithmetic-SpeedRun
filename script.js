...
let equationsArray = []; // stores equations
let playerGuessArray = []; // stores all the trues and falses
let bestScoreArray = []; // save best scores
...
// Time
let timer; // interval for stop and start
...
// Scroll
let valueY = 0; // goes up 80 px at a time
...
function getSavedBestScores() {
    if (localStorage.getItem('bestScores')) {
        ...
        { questions: 15, bestScore: finalTimeDisplay },
...
function select(guessedTrue) {
    ...
    valueY += 80;
    itemContainer.scroll(0, valueY); // 0 horizontal X-axis, this doesn't move
    // Add player guess to GuessArray
...
// Add Equations to Game Page DOM
...
function populateGamePage() {
    ...
    // Create Equations and then Build Elements in DOM
    createEquations();
    equationsToDOM();
...
startForm.addEventListener('click', () => {
    //looping through each radio container
